...
public class DiscoverPenguin : MonoBehaviour {
    ...
    void Update()
    {
        RaycastHit2D rh = Physics2D.Raycast(transform.position, transform.parent.gameObject.GetComponent<PersonMovement>().facingright ? transform.right : -transform.right, 1f, (1 << 8 | 1 << 9 | 1 << 10));
